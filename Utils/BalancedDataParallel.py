...
def scatter_kwargs(inputs, kwargs, target_gpus, chunk_sizes, dim=0):
    ...
    batch_size = list(inputs.values())[0].size(dim)
    if batch_size < len(kwargs):
        inputs.extend([() for _ in range(len(kwargs) - batch_size)])
    elif len(kwargs) < len(inputs):
        kwargs.extend([{} for _ in range(batch_size - len(kwargs))])
    inputs = tuple(inputs)
    kwargs = tuple(kwargs)
...
class BalancedDataParallel(DataParallel):
    ...
    def forward(self, *inputs, **kwargs):
        ...
        batch_size = list(inputs.values())[0].size(self.dim)
        print('len(inputs): ', str(batch_size))
        print('self.device_ids[:len(inputs)]', str(self.device_ids[:batch_size]))
    ...
    def scatter(self, inputs, kwargs, device_ids):
        bsz = list(inputs.values())[0].size(self.dim)
